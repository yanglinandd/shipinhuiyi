            ...
            <Tooltip title={ mute_all ? '解除静音' : '全体静音' } placement="left">
                {
                    ...
                    <img className='mute-action' id='moren' src={get_img_url_by_name('unmute-all-icon')} onClick={this.mute_all_action}/>
                }
            </Tooltip>
        )
    }
...
class Room extends Component {
    ...
    handle_apply_talker(applicat, agreeCallback, refuseCallback) {
        ...
        confirm({
            ...
            onOk: () => {
                agreeCallback(memberId, to_audience_modal)
                return this.handleMute1(memberId)
                // this.mute_all_action
                // return emedia.mgr.muteBymemberId(confrId,memberId)
            }, // 如果主播已满（sdk 内部测试）, to_audience_modal: 为回调函数
            onCancel: () => refuseCallback(memberId),
            cancelText:'拒绝',
            okText:'同意'
        });
    ...
    _get_drawer_component() {
        ...
        let { role } = this.state.user_room;
        console.log('是我是我是我是我是我是我是我',stream_list);
    ...
    _get_video_item(talker_item,index) {
        ...
        return (
            <div
                ...
                style={{display: (member.is_me) ? "block" : talker_item.larger ? 'block' : 'none'}}
            >
                <div className="info">
                    <span className="name">
                        { nickName + (is_me ? '(我)' : '')}
                    ...
                    <div className="wrapper">
                        <img
                            ...
                            onClick={()=>{
                                ...
                                // if(){
                                if(stream_list[0].member.is_me && stream_list[0].member.role === 7){
                                    this.setState({
                                        modalMember:true
                                    })
                                }else{
                                    message.error('只有管理员才有此权限')
                                }
                            ...
                            <div>
                                {
                                    stream_list.map((item,index)=>{
                                        if(item){
                                            return (
                                                <ul key={index} className='memberListUl'>
                                                    <li>
                                                        <span className='span1'>{item.member.nickName + (item.member.is_me ? '(我)' : '')}</span>
                                                        {
                                                            item.member.is_me ? '':
                                                            <div>
                                                                <span><Button onClick={() =>{
                                                                    let { user_room } = this.state;
                                                                    let { memName } = item.member;
                                                                    if(!user_room || !memName){
                                                                        return
                                                                    }
                                                                    emedia.mgr.kickMembersById(user_room, [memName]);
                                                                ...
                                                                <span><Button onClick={() =>{
                                                                    let wrapperItem = document.getElementsByClassName('item v-wrapper')
                                                                    for(let i = 0; i < wrapperItem.length; i++){
                                                                        if(wrapperItem[i].attributes.sid.value === item.stream.id){
                                                                            if(item.videoStep){
                                                                                wrapperItem[i].style.display = 'none'
                                                                                stream_list[index].videoStep = false
                                                                                this.setState({
                                                                                    stream_list:stream_list
                                                                                })
                                                                            }else{
                                                                                wrapperItem[i].style.display = 'block'
                                                                                stream_list[index].videoStep = true
                                                                                this.setState({
                                                                                    stream_list:stream_list
                                                                                })
                                                                            }
                                                                ...
                                                                }}>{item.videoStep ? '关闭摄像头' : '开启摄像头'}</Button></span>
                                                                <span><Button onClick={() => this.handleMute(item)}>{item.stream.aoff?'解除静音':'静音'}</Button></span>
                                                            </div>
                                                        }
                                                    </li>
                                                </ul>
        ...
        return (
            <div style={{width:'100%', height:'100%'}}>
                ...
                >
                    ...
                    <div className="header">
                       <img src={get_img_url_by_name('品牌价值Logo')} />
                    </div>
                    <Form className="login-form" onSubmit={this.join_handle}>
                        ...
                        <img className='PinPai' src={get_img_url_by_name('品牌价值Logo')} />
                        <div className='app-name'>加入会议</div>
                        ...
                        <div className='version-text'></div>
                        {/*Version:{version}*/}
                    </Form>
